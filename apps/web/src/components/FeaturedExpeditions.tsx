...
const FeaturedExpeditions = () => {
    ...
    const isSanityImage = (
        image: Tour["image"]
    ): image is { _type: "image"; asset: { _ref: string; _type: "reference" } } => {
        return (
            typeof image === "object" &&
            image !== null &&
            "_type" in image &&
            ...
            <section className="py-20 bg-gradient-to-b from-green-50 to-orange-50 dark:from-gray-900 dark:bg-gray-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-16">
                        <h2 className="text-4xl font-bold text-green-800 mb-4">Featured Expeditions</h2>
                        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                            Carefully curated adventures that showcase the best of Kenya&apos;s natural wonders
                        </p>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {expeditions.map((expedition, index) => {
                        // ✅ Safely handle both Sanity images and StaticImageData
                        const imageUrl = isSanityImage(expedition.image)
                            ? urlFor(expedition.image).width(800).height(400).url()
                            : (expedition.image as StaticImageData) || defaultImages[index];
                            ...
                            >
                                <div className="relative h-64">
                                    ...
                                    <div className="absolute top-4 left-4">
                                        <Badge
                                            className={`${getDifficultyColor(expedition.difficulty)} font-semibold`}
                                        >
                                            {expedition.difficulty}
                                        </Badge>
                                    </div>
                                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full">
                                        <span className="text-orange-600 font-bold text-lg">${expedition.price}</span>
                                    </div>
                                </div>

                                <div className="p-6">
                                    <h3 className="text-xl font-bold text-green-800 mb-2">{expedition.title}</h3>
                                    <p className="text-gray-600 dark:text-gray-300 mb-4">{expedition.route}</p>
